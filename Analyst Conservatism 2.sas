...
*Winsorization and truncation macro;
...
proc sql;*http://sbaleone.bus.miami.edu/PERLCOURSE/SASFILES/SQL_EXAMPLES.sas;
 create table lagone
 as select a.*, b.lagat, b.lagprc, b.lagcsho
 from compstatret a left join lag b
 on (a.gvkey=b.gvkey) and (a.fyear=b.fyear+1);
quit;
...
proc sort data=allobscomp3 out=testsort; by gvkey merger year post treat; run;

%Winsorize_Truncate(dsetin = testsort, dsetout = winsor, byvar = none, vars = assets, type = W, pctl = 1 99);
...
*Can I show that the result does not exist when we restrict the sample to firms with analyst coverage?;
proc sql;*http://sbaleone.bus.miami.edu/PERLCOURSE/SASFILES/SQL_EXAMPLES.sas;
 create table ibesonly
 as select a.*, b.coverage
 from export2 a left join ibes b
...
proc sort data=ibesonly2; by gvkey year post descending treat; run;
proc sort data=ibesonly2 out=ibesonly3 nodupkey; by gvkey year post; run;
