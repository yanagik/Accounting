...
*Before -1;
proc sql;*http://sbaleone.bus.miami.edu/PERLCOURSE/SASFILES/SQL_EXAMPLES.sas;
 create table tradebefore1
 as select a.*, b.cutv as beforeone, b.raisev as beforeoner
 from postreductionind3 a left join postreductionind3 b
 on (a.sicthree=b.sicthree) and (a.year=b.year-1);
quit;
...
*After 1;
proc sql;*http://sbaleone.bus.miami.edu/PERLCOURSE/SASFILES/SQL_EXAMPLES.sas;
 create table tradeafter1
 as select a.*, b.cutv as afterone, b.raisev as afteroner
 from tradezero a left join tradezero b
 on (a.sicthree=b.sicthree) and (a.year=b.year+1);
quit;

*After 2;
proc sql;*http://sbaleone.bus.miami.edu/PERLCOURSE/SASFILES/SQL_EXAMPLES.sas;
 create table tradeafter2
 as select a.*, b.cutv as aftert, b.raisev as aftertr
 from tradeafter1 a left join tradeafter1 b
 on (a.sicthree=b.sicthree) and (a.year=b.year+2);
quit;
...
data tradeafter3;
 ...
 by sicthree;
 retain aftertwo;
 if first.sicthree then aftertwo=0;
 if aftert=1 then aftertwo=1;
 retain aftertwor;
 if first.sicthree then aftertwor=0;
 if aftertr=1 then aftertwor=1;
run;

data trade2;
 set tradeafter3;
 if beforeone=. then beforeone=0;
 if beforezero=. then beforezero=0;
 if afterone=. then afterone=0;
 if beforeoner=. then beforeoner=0;
 if beforezeror=. then beforezeror=0;
 if afteroner=. then afteroner=0;
run;

*Export;
data temp.schotttrade6;
 set trade2;
run;
