...
use /mnt/home/ay32/STATA/cscore.dta
...
estpost sum coverage cscore treat assets mtb lev, detail listwise
esttab, cells("mean sd min p50 max")
esttab using /mnt/home/ay32/STATA/descscore.tex, replace label cells("mean(fmt(2)) sd(fmt(2)) min(fmt(2)) p50(fmt(2)) max(fmt(2))") booktabs nonum nomtitles f gaps plain
...
use /mnt/home/ay32/STATA/conacc.dta
...
estpost sum coverage con treat assets mtb lev, detail listwise
esttab, cells("mean sd min p50 max")
esttab using /mnt/home/ay32/STATA/desconacc.tex, replace label cells("mean(fmt(2)) sd(fmt(2)) min(fmt(2)) p50(fmt(2)) max(fmt(2))") booktabs nonum nomtitles f gaps plain
...
// esttab using /mnt/home/ay32/STATA/conall.tex, label se star(* 0.10 ** 0.05 *** 0.01) ar2 replace f cells(b(fmt(3)star) se(par fmt(3)))
eststo clear
