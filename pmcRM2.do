...
eststo: quietly areg dprod bench postreductionv benchpostred size mtb roa i.year, absorb(gvkey) r cl(sicthree)
...
eststo: quietly areg dprod bench postincreasef benchpostincf size mtb roa i.year, absorb(gvkey) r cl(sicthree)
esttab, label se star(* 0.10 ** 0.05 *** 0.01) ar2 indicate(Year Fixed Effects = *year) replace f cells(b(fmt(3)star) se(par fmt(3)))
esttab using /mnt/home/ay32/STATA/csrprodpostinc.tex, label se star(* 0.10 ** 0.05 *** 0.01) ar2 indicate(Year Fixed Effects = *year) replace f cells(b(fmt(3)star) se(par fmt(3)))

eststo clear

// Robustness check, split by z-score
eststo: quietly areg dprod bench postreductionv benchpostred size mtb roa i.year if zrank==0, absorb(gvkey) r cl(sicthree)
eststo: quietly areg dprod bench postreductionv benchpostred size mtb roa i.year if zrank==1, absorb(gvkey) r cl(sicthree)
...
eststo: quietly areg drd bench postreductionv benchpostred size mtb roa i.year, absorb(gvkey) r cl(sicthree)
...
// Robustness check, split by z-score
eststo: quietly areg drd bench postreductionv benchpostred size mtb roa i.year if zrank==0, absorb(gvkey) r cl(sicthree)
eststo: quietly areg drd bench postreductionv benchpostred size mtb roa i.year if zrank==1, absorb(gvkey) r cl(sicthree)
...
filefilter /mnt/home/ay32/STATA/csrprodpostinc.tex /mnt/home/ay32/STATA/csrprodpostinc2.tex, from("\BS_{") to ("_{") replace
